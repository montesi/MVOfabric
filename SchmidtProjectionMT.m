...
for icase=1:ncase;
    ...
    % plot the contours
    figure(ifig); subplot (nsy,nsx,icase); hold on;
...
for ic=1:numel(fc); 
    ...
    plot(fc(ic).contour.lower.x,fc(ic).contour.lower.y,'color',colNow,'LineWidth',1)
end
axis equal;
plotAddSchmidtGrid(gca,dAngle);  %reference frame
...
% set(gca,'Clim',[0,fmax]);
colorbar('Ticks',flipud(unique([fc.level])),'fontsize',12);
end
...
for iSpokes=1:nSpokes-1;
    ...
    text(stretch*sqrt(2)*cosd(thNow),...
        ...
        'HorizontalAlignment',HA,'fontsize',18)
end
...
for iCircle=1:nCircles
    ...
    text(stretch*rhoNow*cosd(thNow),stretch*rhoNow*sind(thNow),...
        ...
        'HorizontalAlignment',HA,'VerticalAlignment','bottom','fontsize',18)
end
end
