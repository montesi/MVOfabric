function grid=SchmidtProjection(data,ifig,iDet,dAngle,nx);%,dataPerc,BDW)
...
figure(iDet); clf; 
[f,Cn,fc]=densityWatsonKernel(data.XYZ,data.Weight,grid,1.5,10,iDet);
grid.fWatson=f;grid.fcWatson=fc;grid.CnWatson=Cn;
% plot the contours
figure(ifig); subplot 132; hold on;
...
for ic=1:numel(fc); 
    ...
    % plot(fc(ic).contour.x,fc(ic).contour.y,'-')
    plot(fc(ic).contour.lower.x,fc(ic).contour.lower.y,'color',colNow)
    %patch(fc(ic).contour.x,fc(ic).contour.y,fc(ic).level)
     % patch(fc(ic).contour.lower.x,fc(ic).contour.lower.y,fc(ic).level)
end
axis equal;
plotAddSchmidtGrid(gca,dAngle);  %reference frame
if ~isempty(data.orientation);
    addOrientation(data.orientation,gca);
end
text(0,1.7,sprintf('Watson kernel, C=%g',Cn),...
    'FontWeight','bold',...
    'HorizontalAlignment','center',...figure
    'fontsize',12)
set(gca,'Visible','off','clim',[0,fMax])
% set(gca,'Clim',[0,fmax]);
colorbar('Ticks',flipud(unique([fc.level])));

% [f,Cn,fc]=densityWatsonKernel(data.XYZ,2*ones(size(data.Phi)),grid,1.5,10,2);


%% Density contours, Fisher kernel
% [f,Cn,fc]=densityFisherKernel(data.XYZ,grid,2,10,4);
%% Density contours, Watson without weight
[f,Cn,fc]=densityWatsonKernel(data.XYZ,ones(size(data.Phi)),grid,1.5,10,iDet);
%[f,Cn,fc]=densityWatsonKernelSafe(data.XYZ,grid,1.5,10,4);
grid.fWatsonNoWeight=f;grid.fcWatsonNoWeight=fc;grid.CnWatsonNoWeight=Cn;

% plot the contours
figure(ifig); subplot 133; hold on;
% figure(3); clf; hold on;
colMap=colormap(sky);
fMax=max(f(:)); 
colInd=linspace(0,fMax,size(colMap,1));
...
plotAddSchmidtGrid(gca,dAngle);  %reference frame
if ~isempty(data.orientation);
    addOrientation(data.orientationNW,gca);
end
% text(0,1.7,sprintf('Fisher kernel, C=%g',Cn),...
text(0,1.7,sprintf('Non-weighted, C=%g',Cn),...
    'FontWeight','bold',...
    'HorizontalAlignment','center',...figure
    'fontsize',12)
set(gca,'Visible','off','clim',[0,fMax]);
