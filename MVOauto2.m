...
for iRepo=1:nRepo;
    if ~isempty(data(iRepo).Pore)
        ...
        [data(iRepo).Pore,ORn]=analyseSequence(data(iRepo).Pore,min((iRepo+1)/nRepo,1));
...
%%
% Filter the data
for fBuffer=[1,1.5,2]; %Factor above critical values
    clear ORgood;
    for iRepo=1:nRepo;
        if ~isempty(data(iRepo).Pore);
            [F,N,E]=fileparts(data(iRepo).Name);
            fprintf('\n\n\nWorking on %s, buffer %g\n',N,fBuffer);
            oddShape=find(data(iRepo).Pore.Shape<1);
            Vmin=max(data(iRepo).Pore.Volume(oddShape)); %largest volume with impossible shape
            iKeep=find(and(data(iRepo).Pore.Shape>fBuffer,data(iRepo).Pore.Volume>fBuffer*Vmin));
            if isfield(data(iRepo).Pore,'Label')
                data(iRepo).PoreGood.Label=data(iRepo).Pore.Label+"Clean";
            end
            data(iRepo).PoreGood.XYZ=data(iRepo).Pore.XYZ(:,iKeep);
            data(iRepo).PoreGood.Phi=data(iRepo).Pore.Phi(iKeep);
            data(iRepo).PoreGood.Theta=data(iRepo).Pore.Theta(iKeep);
            data(iRepo).PoreGood.Volume=data(iRepo).Pore.Volume(iKeep);
            data(iRepo).PoreGood.Shape=data(iRepo).Pore.Shape(iKeep);
            data(iRepo).PoreGood.Weight=data(iRepo).Pore.Weight(iKeep);

            [data(iRepo).PoreGood,ORn]=analyseSequence(data(iRepo).PoreGood,iRepo/nRepo);
            ORgood(1,iRepo)=ORn(1);ORgood(2,iRepo)=ORn(2);
        end
    end
    %
    plotOrientation(ORgood,6);
    print(6,'-dpdf',sprintf('Shape_Filtered%g',fBuffer));
    disp('All done!')
end
%%
...
data.case(2).weight=data.Volume; data.case(2).label='Data weighted by volume';data.case(2).case='Volume';
...
grid=SchmidtProjectionGeneral(data,1,3,30,50,OR);
print(1,'-dpdf',sprintf('%s-%s','CQ0705',data.Label),'-fillpage');
end
