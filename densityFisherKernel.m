function [f,Cn,fc]=densityFisherKernel(XYZ,grid,r,M0,ifig);
% ifig=4;
...
if nPt>1000;
    ...
    for iIter=1:nIter;
        CnIter(iIter)=smoothingSelfConsistent(XYZ(:,ceil(nPt*rand(1,sIter))),ifig);
    end
    ...
    Cn=round(smoothingSelfConsistent(XYZ,ifig),1,'significant');
end
Cn=min(Cn,100);
...
f=densityEstimate(XYZ,grid.XYZ,Cn);
...
while nM>=2; % there is at least one more contour;
    ...
    function Cn=smoothingSelfConsistent(P3,ifig);
        n=size(P3,2);
        ...
        while ~CV
            ...
            for iC=1:numel(CAll);
                Lstore(iC)=pseudoLogLikelihood(cosTheta,CAll(iC),n);
            end
            plot(CAll,Lstore,'.-')
    ...
    function L=pseudoLogLikelihood(P,C,n);
        ...
        for i=1:n;
            iInc=[1:i-1,i+1:n];
            fIncomplete(i)=sum(W2(i,iInc));
        end
        L=sum(log(fIncomplete));
    end
end

function f=densityEstimate(XYZ,XYZi,C);
n=size(XYZ,2);
